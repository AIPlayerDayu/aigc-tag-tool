...
一次可拖入多张图片 / 多个视频，逐个检测；顶部汇总栏显示「🔴 AI / 🟠 疑似 / 🟢 未发现」的数量，并可**导出检测报告 (CSV)**（文件名 / 判定 / 证据），方便留档或核对一批素材。

## 去除 AI / AIGC 标识（仅元数据）

检出为 🔴/🟠 的文件下方会出现「🧹 去除 AI / AIGC 标识并保存」按钮：

- **视频**：`ffmpeg -map_metadata -1 -c copy` 去掉容器里的全部标签（含 AIGC），**流拷贝、不重新编码 → 画质无损**。
- **图片**：纯 Python 无损剥离全部元数据（EXIF/XMP/IPTC/C2PA），无需 exiftool。
- 处理完会**自动复检**并显示结果。

> 适用场景：给自己的图片/视频做了美颜/剪辑，却被自动打上 AIGC 标识，想去掉这个（针对自己内容的）标签。
> ⚠️ **本功能只移除元数据，不改动画面像素**。若平台是靠画面本身判定，删元数据无效——见下方[「为什么平台仍可能识别」](#为什么删了元数据平台仍可能识别为-ai)。
...
## 为什么删了元数据，平台仍可能识别为 AI

AI 内容的标识分**四层**，本工具只处理最外面的元数据层：

| 层级 | 藏在哪 | 本工具 |
|------|--------|--------|
| ① 元数据标识 | 容器标签 / EXIF / XMP（AIGC、C2PA、DigitalSourceType…） | ✅ 能去除 |
| ② 视频码流 SEI | 编码后的比特流里（`-c copy` 碰不到） | ❌ 需重新编码 |
| ③ 隐形像素水印 | 直接嵌进像素（如 Google **SynthID**、部分国产模型） | ❌ 删元数据无效 |
| ④ 平台自家 AI 画面识别 | 平台服务器跑模型分析**画面本身** | ❌ 与文件里有无标识无关 |

如果去了元数据、平台**仍然**判定为 AI（尤其是上传数小时后才被标记），基本就是 ③④——
平台在用**画面识别模型**看内容本身，这跟文件里有没有元数据标识毫无关系。**删元数据在这种情况下不会有任何效果。**

本工具的定位是**检测**、以及移除**针对自己内容的**元数据标识；它不提供、也无法可靠地绕过平台的画面级 AI 识别。
